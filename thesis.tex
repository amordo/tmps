...
Scala Native~\cite{scala_native} compiles Scala to native code via \gls{LLVM}.
It exposes \texttt{Zone}-based off-heap allocation: memory is obtained from the
operating system, bypassing the \gls{GC}, and is freed deterministically when the
zone exits scope.
...
% TODO: hardware, software versions, dataset descriptions
